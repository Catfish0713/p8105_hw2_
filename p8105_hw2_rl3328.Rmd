...
baseline = read_csv("./data_mci/MCI_baseline.csv",col_names = FALSE) |>
  slice(-c(1,2)) |>
...
* `r nrow(baseline)` participants were recruited, and of these `r baseline |> filter(!is.na(age_at_onset)) |> nrow()` develop MCI? What is the average baseline age? What proportion of women in the study are APOE4 carriers?

```{r}
amyloid = read_csv("./data_mci/mci_amyloid.csv",col_names = FALSE) |>
  slice(-c(1,2)) |>
